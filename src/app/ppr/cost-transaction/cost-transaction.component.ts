...
export class CostTransactionComponent implements OnInit {
 ...
 displaydateandyear
  ...
  bsfromDropdown(){
    ...
    this.cost_transfer.get('allocationbsform_filter').valueChanges
      .pipe(
        ...
        switchMap(value => this.pprserice.get_bs_dropdown(1,value,1)
          .pipe(
            finalize(() => {
  ...
  private getasset_Bs(keyvalue) {
    this.pprserice.get_bs_dropdown(1,keyvalue,1)
      .subscribe((results: any[]) => {
        let datas = results["data"];
        this.Bsfromcode = datas;
            ...
            .subscribe(x => {
              ...
              if (atBottom) {
                if (this.has_nextval === true) {
                  console.log("true")
                  this.pprserice.get_bs_dropdown(1,this.bsformInput.nativeElement.value, this.currentpagenum+1)
                    .subscribe((results: any[]) => {
                      let datas = results["data"];
  ...
  ccfromDropdown(){
    ...
    this.cost_transfer.get('allocationfromcc').valueChanges
      .pipe(
        ...
        switchMap(value => this.pprserice.get_cc_dropdown(1,value,1)
          .pipe(
            finalize(() => {
              // console.log(value.id)
              // this.val=value
  ...
  private getasset_cc(keyvalue) {
    this.pprserice.get_cc_dropdown(1,keyvalue,1)
      .subscribe((results: any[]) => {
        let datas = results["data"];
        this.ccfromcode = datas;
         ...
         fromEvent(this.ccfromAutoComplete.panel.nativeElement, 'scroll')
           ...
           .subscribe(x => {
             ...
             if (atBottom) {
               if (this.has_nextval === true) {
                 console.log("true")
                 this.pprserice.get_cc_dropdown(1,this.ccformInput.nativeElement.value, this.currentpagenum+1)
                   .subscribe((results: any[]) => {
                     let datas = results["data"];
                     let datapagination = results["pagination"];
                     console.log("loop",this.currentpagenum)
