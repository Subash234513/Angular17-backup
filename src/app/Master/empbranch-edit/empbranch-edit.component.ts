...
@Component({
  selector: 'app-empbranch-edit',
  templateUrl: './empbranch-edit.component.html',
  styleUrls: ['./empbranch-edit.component.scss']
})
export class EmpbranchEditComponent implements OnInit {
  ...
  minLength:number=10;
  maxLength:number=10;
  contactTyped={5:'EMPLOYEE',10:'GROUP123'};
  reg:any=new RegExp('^[0-9]{2}[A-Z]{5}[0-9]{4}[A-Z]{1}[1-9A-Z]{1}Z[0-9A-Z]{1}$');
  constructor(private shareservice:ShareService,private spinner:NgxSpinnerService,private datepipe:DatePipe,private fb:FormBuilder,private masterservice:masterService,private notification:NotificationService) { }
  ...
  ngOnInit(): void {
   ...
   this.dataid=+this.shareservice.branchmstid.value;
   this.masterservice.getbranchdataid(this.dataid).subscribe(d=>{
     let dts:any=d;
     this.createemployss.patchValue({
        'code':dts['code'],
        'name':dts['name'],
        'tanno':dts['tanno']?dts['tanno']:'',
        'glno':dts['glno']?dts['glno']:'',
        'stdno':dts['stdno']?dts['stdno']:'',
        'incharge':{'full_name':dts['incharge']},
        'gstin':dts['gstin'],
        'branch':{'id':dts['control_office_branch']['id'],'name':dts['control_office_branch']['name']},
        'line1':dts['address']['line1'],
        'line2':dts['address']['line2'],
        'line3':dts['address']['line3'],
        'pincode':{'id':dts['address']['pincode']['id'],'no':dts['address']['pincode']['no']},
        'city':{'id':dts['address']['city']['id'],'name':dts['address']['city']['city_name']},
        'district':{'id':dts['address']['district']['id'],'name':dts['address']['district']['name']},
        'state':{'id':dts['address']['state']['id'],'name':dts['address']['state']['name']},
        'contacttype':this.contactTyped[dts['contact']['type_id']],
        'personname':dts['contact']['name'],
        'condesignation':{'id':dts['contact']['designation']['id'],'name':dts['contact']['designation']['name']},
        'landline1':dts['contact']['landline'],
        'landline2':dts['contact']['landline2'],
        'contactnumber':dts['contact']['contactnumber'],
        'contactnumber2':dts['contact']['contactnumber2'],
        'conemailid':dts['contact']['email'],
        'condob':dts['contact']['dob'],
        'conwedday':dts['contact']['wedding_date']
     },
  ...
  // };
   let data:any={
      ...
      "incharge":this.createemployss.get('incharge').value.name,
  ...
  this.spinner.show();
    this.masterservice.getemployeebranch(data).subscribe(datas=>{
      ...
      if(data['status']=='success'){
        this.notification.showSuccess(data['description']);
        this.onSubmit.emit();
      }
      else{
