...
export class BranchactivityEditComponent implements OnInit {
  ...
  @ViewChild('activityname') activitynameautocomplete: MatAutocomplete;
  @ViewChild('activitynameinput') activitynameinput: any;

  activitydropdown: Array<activity>;
  activity_drop_hasnext = true;
  activity_drop_hasprevious = true;
  activity_drop_currentpage = 1;
  ...
  ngOnInit(): void {
    ...
    this.getactivitydropdown('', this.activity_drop_currentpage)
  ...
  categoryname(){
    ...
    this.ActivityEditForm.get('rel_type').valueChanges
      .pipe(
        debounceTime(100),
        distinctUntilChanged(),
        tap(() => {
  ...
  get categorytype() {
    return this.ActivityEditForm.get('rel_type');
  }

  public displayFnactivity(activity?: activity): string | undefined {
    return activity ? activity.name : undefined;
  }

  get activity() {
    return this.ActivityEditForm.get('activity_id');
  }

  getactivitydropdown(value, page) {

    this.atmaService.getactivitydropdown(value,page)
      .subscribe((results: any[]) => {

        let datas = results["data"];
        let pagination = results['pegination']
        this.activitydropdown = datas;
        if (datas.length != 0) {
          this.activity_drop_hasnext = pagination.has_next
          this.activity_drop_hasprevious = pagination.has_previous
          this.activity_drop_currentpage = pagination.index
        }
  ...
  activitiyscroll() {
    setTimeout(() => {
      if (
        this.activitynameautocomplete &&
        this.autocompleteTrigger &&
        this.activitynameautocomplete.panel
      ) {
        fromEvent(this.activitynameautocomplete.panel.nativeElement, 'scroll')
          .pipe(
            map(x => this.activitynameautocomplete.panel.nativeElement.scrollTop),
            takeUntil(this.autocompleteTrigger.panelClosingActions)
          )
          .subscribe(x => {
            const scrollTop = this.activitynameautocomplete.panel.nativeElement.scrollTop;
            const scrollHeight = this.activitynameautocomplete.panel.nativeElement.scrollHeight;
            const elementHeight = this.activitynameautocomplete.panel.nativeElement.clientHeight;
            const atBottom = scrollHeight - 1 <= scrollTop + elementHeight;
            if (atBottom) {
              if (this.activity_drop_hasnext === true) {
                this.atmaService.getactivitydropdown(this.activitynameinput.nativeElement.value,this.activity_drop_currentpage+1)
                  .subscribe((results: any[]) => {
                    let datas = results["data"];
                    let pagination = results['pegination']
                    this.activitydropdown = datas;
  ...
  getActivityEdit() {
    ...
    let Reltype = data.rel_type;
    let  Activityid = data.activity_id;
  ...
  submitForm() {
    ...
    if(this.ActivityEditForm.value.start_date != "None"){
     ...
     if(typeof(dateValue.activity_id)== 'object'){
      dateValue.activity_id =  dateValue?.activity_id?.id
     }else if(typeof(dateValue.activity_id)== 'number'){
      dateValue.activity_id =  dateValue.activity_id 
     }else{
      this.ActivityEditForm.value.activity_id = null 
     }

     if(typeof(dateValue.rel_type)== 'object'){
      dateValue.rel_type =  dateValue?.rel_type?.id
     }else if(typeof(dateValue.rel_type)== 'number'){
      dateValue.rel_type =  dateValue.rel_type 
     }else{
      this.ActivityEditForm.value.rel_type = null 
     }
