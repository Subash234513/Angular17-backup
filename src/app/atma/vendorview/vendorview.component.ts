...
// risk summary
  ...
  tabchange(event) {
    ...
    if (event.isUserInput == true) {
      ...
      if (this.statusTab === 'BRANCH DETAILS') {
        ...
        this.isPortalEditForm = false;
        return false
      }else if(this.statusTab=='PORTAL USER'){
        this.isPortal = true;
        this.isPortalForm = false;
        this.isPortalEditForm = false;
        this.isRisk = false;
  ...
  gettransactionsummary(pageNumber = 1, pageSize = 10) {
    ...
    this.atmaService.gettransactionsummary(transactionDetail, pageNumber, pageSize)
      .subscribe((result) => {
  ...
  addportal(){
    (this.vendorData.code)? this.isPortalForm=true:this.isPortalForm=true
    this.isPortal = true;
    this.shareService.vendorID.next(this.vendorId)
  }
  ...
  evaluationroute(value) {
    console.log("vendorEdit-this.vendorIdForEdit", this.vendorIdForEdit)
    if(value ==0){
    this.router.navigate(['/atma/evaluation'],{ queryParams: {"vendorid": btoa(this.vendorIdForEdit),"vendor_or_activity":btoa(value) }} )
    }
    else{
