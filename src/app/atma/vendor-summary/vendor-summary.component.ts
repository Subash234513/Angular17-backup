...
export class VendorSummaryComponent implements OnInit {
  ...
  rmname(){
    let rmkeyvalue: String = "";
      ...
      routingevaluation(value,avt_or_vend){
        if(avt_or_vend == 0){
          this.router.navigate(['/atma/evaluation'],{ queryParams: {"vendorid": btoa(value),"vendor_or_activity":btoa(avt_or_vend) }} )

        }
        else{
        this.router.navigate(['/atma/activity_questionnaire'],{ queryParams: {"vendorid": btoa(value),"vendor_or_activity":btoa(avt_or_vend) }} )
      ...
      clicktoinactive(vendor){
        var answer = window.confirm("Are you sure to InActivate Portal?");
        if(answer){
      ...
      clicktoactive(vendor){
        var answer = window.confirm("Are you sure to Activate Portal?");
        if(answer){
        let json = {
          "id" :vendor?.id,
          "name":vendor?.name,
          "supplier_code":vendor?.code,
          "relationship_type":vendor?.group,
          "created_by":vendor?.created_by?.id
        }
       this.atmaService.PortalActiveForm(json)
        .subscribe(result =>{
