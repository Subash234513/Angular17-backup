...
if(this.apSubCategoryForm.value.assetcode.toString().length==5){
console.log('Done');
}
else{
  this.notification.showError('Please Enter The Asset Code 5 Character');
  return false;
}
...
kyenbdata_new(event:any){
  let d:any=new RegExp(/[!@#$%^&*()_+\-=\[\]{};':"\\|,.<>\/?]+1234567890/)
  console.log(d.test(event.key))
  if(d.test(event.key)==true){
    return false;
  }
