...
@Component({
  selector: 'app-jv-approvalview',
  templateUrl: './jv-approvalview.component.html',
  styleUrls: ['./jv-approvalview.component.scss'],
  providers: [
...
export class JvApprovalviewComponent implements OnInit {

  approverviewForm:FormGroup
  ...
  ngOnInit(): void {
    ...
    this.approverviewForm = this.fb.group({
  ...
  getjvview(){
    ...
    .subscribe(result => {
      ...
      this.approverviewForm.patchValue({
        crno:result?.jecrno,
  ...
  jvapprove(){
    ...
    let data = this.SubmitApproverForm?.value
    if( data?.remark == ""||data?.remark == undefined || data?.remark == null){
    ...
    let json = {
      "id":this.jvid,
      "jedescription":data?.remark
    }
    this.spinnerservice.show()
    this.jvservice.jvapprove(json)
    .subscribe(res=>{
      if(res.status == "success"){
      this.notification.showSuccess("JV Approved Successfully")
      this.spinnerservice.hide()
      this.onSubmit.emit()
      }else{
  ...
  jvreject(){
    ...
    this.spinnerservice.show()
    this.jvservice.jvreject(json)
    .subscribe(res=>{
      
      if(res.status == "success"){
      this.notification.showSuccess("JV Rejected Successfully")
      this.spinnerservice.hide()
      this.onSubmit.emit()
      }else{
        this.notification.showError(res.description)
