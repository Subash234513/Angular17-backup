    ...
    this.activatedroute.queryParams.subscribe(
      params => {
        if(params)
        {
          ...
          console.log("this.supplierGSTflag -->", this.supplierGSTflag)   
  ...
  getApHdr()
  {
    ...
    .subscribe(result => {  
      ...
      { 
        ...
        console.log("this.raisedby-----", this.raisedbyid)
        ...
        console.log("this.invdtls----",this.invdtls)
      }
      else
      {
     ...
     if (this.invHdrRes !==undefined && this.invHdrRes!==null)
      {
        this.frmInvHdr.patchValue(
          {
            ...
            taxableAmt:this.invHdrRes.invoice_amount,
        ...
        this.gsttype = this.invHdrRes.gsttype
              ...
              if (result)
                {
                  ...
                  let invCreditList1 = this.invCreditList.filter(x => (x.is_display == "YES" && x.amount>0));
...
getcreditrecords(datas) {
  ...
  for (let data of creditdet) {
    ...
    if(i==0)
    {
      this.creditamt1 = data.amount
      console.log("this.creditamt1---",this.creditamt1)
    ...
    // if(data.paymode.id == 7)
    // {
    //   this.taxableamount += data.taxableamount
    //   this.frmInvHdr.get('taxableAmt').setValue(this.taxableamount)
    // }
    ...
    else
    {
      ...
      {
        amountchange.setValue(data.amount)  
  ...
  // console.log("dta",dta)
  // if (dta.length === 1) {
    this.creditamt1  = this.totalamount
...
INVdetail() {
  ...
  // gethsncode2(prod) {
  //   this.hsnpercent = prod.hsn_id.igstrate
  //   this.hsncode = code
  //   console.log("hsn_code",hsn_code)
  //   this.InvoiceDetailForm.get('invoicedtls')['controls'][ind].get('hsn').setValue(hsn_code)
  //   this.InvoiceDetailForm.get('invoicedtls')['controls'][ind].get('hsn_percentage').setValue(this.hsnpercent)
  //   // this.getgst1(hsn_code,ind)
  //   this.getgst(ind)
  // }
  ...
  INVdatasums() {
    this.INVamt = this.InvoiceDetailForm.value['invoicedtls'].map(x =>Number((x.totalamount)));
  ...
  getdebitrecords(datas) {
    ...
    else
    {
      ...
      for (let debit of datas) {  
        ...
        let code = (debit.category_code.code).toLowerCase()
        if(code == "dummy" || code.indexOf("unexpected error") >= 0 || code.indexOf("unexpected_error") >= 0)
        {
        ...
        let subcode = (debit.subcategory_code.code).toLowerCase()
        if(subcode == "dummy" || subcode.indexOf("unexpected error") >= 0 || subcode.indexOf("unexpected_error") >= 0)
        {
  ...
  addcreditSection() {
    console.log("this.bankdetailsids---",this.bankdetailsids)
  ...
  CreditDessss(pay, index, taxtype ="", getcred = false) {
    ...
    if (this.paymodeid === 5 || this.paymodeid === 8 || this.paymodeid === 4 || this.paymodeid === 1 )
     {
       if(this.paymodeid != 4)
       {
        this.getaccno(this.paymodeid,index)     
       }
       else
       {
        this.getERA(index)
     ...
     {
      if(this.paymodeid == 2 || this.paymodeid == 6 || this.paymodeid == 7 )
      {
  ...
  getcreditpaymodesummary(index) {
    ...
    this.service.getcreditpaymodesummaryy(1, 10, this.paymodeid, this.suppid, this.accountnumber)
      .subscribe((results: any[]) => {
        if(results)
        {
          let datas = results["data"];
...
getEraAccDet(acc,ind)
{
           ...
           if(this.supplierdata.isexcempted === "Y")
           {
             ...
             this.creditdetForm.patchValue(
               {
                 ...
                 thresholdValue : this.supplierdata.excemthrosold_amt,
                 amountPaid : this.supplierdata.paid_invoiceamount,
             ...
             if(getcred == false) 
             {
              if(this.supplierdata.is_throsold == false) 
              {
                ...
                {
                  tdsCalc = true
                  this.InvoiceDetailForm.get('creditdtl')['controls'][ind].get('taxableamount').setValue(this.taxableamount)    
                  this.gettaxcalc(ind)                
                }
                else
                {
                  tdsCalc = true
                  this.InvoiceDetailForm.get('creditdtl')['controls'][ind].get('taxableamount').setValue(this.supplierdata.current_tds_applicable_amt)
    ...
    if (taxrate != undefined || taxrate != "" || taxrate != null || taxableamt != undefined || taxableamt != "" || taxableamt != null) {
      ...
      this.service.gettdstaxcalculation(taxableamt, taxrate)
        .subscribe(results => {
          ...
          this.InvoiceDetailForm.get('creditdtl')['controls'][index].get('amountchange').setValue(Math.round(amount))
          if(exceedAmt >0)
          {
            this.addcreditSection()
            let paymodetds = this.paymodesList.filter(x => x.id == 7)[0]
            this.InvoiceDetailForm.get('creditdtl')['controls'][index+1].get('paymode_id').setValue(paymodetds)
            let type = this.InvoiceDetailForm.value.creditdtl[index].suppliertaxtype
            console.log("type----", type)
            this.InvoiceDetailForm.get('creditdtl')['controls'][index+1].get('suppliertaxtype').setValue(type.subtax.name + " - " + type.subtax.glno)
            this.InvoiceDetailForm.get('creditdtl')['controls'][index+1].get('taxableamount').setValue(exceedAmt)
            this.InvoiceDetailForm.get('creditdtl')['controls'][index+1].get('suppliertaxrate').setValue(type.taxrate.rate)
            this.InvoiceDetailForm.get('creditdtl')['controls'][index+1].get('creditglno').setValue(type.subtax.glno)
            
            let taxableamt2 = this.InvoiceDetailForm.value.creditdtl[index+1].taxableamount
            let taxrate2 = this.InvoiceDetailForm.value.creditdtl[index+1].suppliertaxrate
            this.service.gettdstaxcalculation(taxableamt2, taxrate2)
            .subscribe(result => {
              let amt = result.tdsrate
    
              this.InvoiceDetailForm.get('creditdtl')['controls'][index+1].get('amountchange').setValue(Math.round(amt))
            })
          }
    ...
    for (let i in ppxForm)
     ...
     if (ppxselected == true)
     {
       ...
       else
       {
        ...
        this.InvoiceDetailForm.get('creditdtl')['controls'][this.getcreditindex].get('amountchange').setValue(Number(this.selectedppxdata[n].liquidate_amt))
        let ppxcontrol = this.ppxForm.controls["ppxdtl"] as FormArray;
        ppxcontrol.clear()
  ...
  ppxamt(e,ind)
  {
    let amt = String(this.ppxForm.value.ppxdtl[ind].liquidate_amt)
    let balamt = this.ppxForm.value.ppxdtl[ind].ppxheader_balance
    if (e > balamt || e > this.totalamount) 
    {
      let n = amt.slice(0,amt.length-1)
      this.ppxForm.get('ppxdtl')['controls'][ind].get('liquidate_amt').setValue(Number(n)) 
      this.notification.showError("Liquidate amount should not exceed the invoice amount and Balance amount.")
    }   
  }
    ...
    for (let i in ppxForm)
     {
       if(ppxForm[i].select == true)
       {
        this.ppxsum +=Number(ppxForm[i].liquidate_amt)
       }
     }
  }
  ...
  amountReduction() {
    ...
    {
      if(creditForm[i].amountchange > 0 && creditForm[i].creditglno != "151515" && creditForm[i].creditglno != "151516" && creditForm[i].creditglno != "151517")
      {
        reductionSum += creditForm[i].amountchange
        this.InvoiceDetailForm.get('creditdtl')['controls'][i].get('amount').setValue(creditForm[i].amountchange)
      }
      else
      {
        this.amtChangeFlag[i] =false     
    ...
    this.InvoiceDetailForm.get('creditdtl')['controls'][0].get('amount').setValue(Number(this.invHdrRes.totalamount - reductionSum))
  ...
  creditdatasums() {
    ...
    this.cdtsum = this.cdtamt.reduce((a, b) => Number(a) + Number(b), 0);
    ...
    this.service.invCreditAddEdit(this.apinvHeader_id, detaildata)
        .subscribe(result => {
          ...
          if (result.code != undefined) {
            ...
            let creditresult = result["data"]
            console.log("credit result--", creditresult)

            let i = 0
            let ppxdet  = []
            for (let x in creditresult) {
              if(creditresult[x].paymode == 6)
                {
                  let ppxdetailid 
                  let id = creditresult[x].id
                  ...
                  if (ppxdetailid != undefined)
                  {
                    ppxdata = { "apppxheader_id" : this.selectedppxdata[i].apppxheader_id,
                                  ...
                                  "apcredit_id" : id,
                                  "ppxdetails_amount" : this.selectedppxdata[i].ppxheader_amount,
                                  "ppxdetails_adjusted" : this.selectedppxdata[i].liquidate_amt,
                                  "ppxdetails_balance" : Number(ppx.ppxdetails_balance)+ Number(ppx.ppxdetails_adjusted)-Number(this.selectedppxdata[i].liquidate_amt),
                                  "ap_amount" : this.selectedppxdata[i].liquidate_amt,
                  ...
                  else
                  {
                    ppxdata = { "apppxheader_id" : this.selectedppxdata[i].apppxheader_id,
                                  ...
                                  "ppxdetails_amount" : this.selectedppxdata[i].ppxheader_amount,
                                  "ppxdetails_adjusted" : this.selectedppxdata[i].liquidate_amt,
                                  "ppxdetails_balance" : Number(this.selectedppxdata[i].ppxheader_balance)-Number(this.selectedppxdata[i].liquidate_amt),
                                  "ap_amount" : this.selectedppxdata[i].liquidate_amt,
                                  "ppxlique_crno" : this.crno
                                }
                  }
  ...
  deletecreditdetail(section, ind) {
    let id = section.value.id
    let amountchange = this.InvoiceDetailForm.get('creditdtl')['controls'][ind].get('amountchange').value
    let creditamount1 = this.InvoiceDetailForm.get('creditdtl')['controls'][0].get('amount').value
    ...
    this.InvoiceDetailForm.get('creditdtl')['controls'][0].get('amount').setValue(Number(creditamount1) + Number(amountchange))
  }
  ...
  debitform() {
    ...
    for (let i in dbtdetaildata) {
      ...
      if ((dbtdetaildata[i].bsproduct_code == '') || (dbtdetaildata[i].bsproduct_code == null) || (dbtdetaildata[i].bsproduct_code == undefined)) {
          ...
          if (result.code != undefined) {
            ...
            .subscribe(result => {
             ...
             if (result)
             {
                ...
                console.log(" Debit data ... invdtladdonid(", this.invdtladdonid ,") ",  this.debitdata )
...
deleteccbs(section, ind) {
  ...
  {
    let dataOnDetails = this.DebitDetailForm.value.debitdtl
    let amt: any = +dataOnDetails[ind].amt
