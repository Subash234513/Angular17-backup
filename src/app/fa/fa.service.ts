...
export class faservice {
  ...
  constructor(private http: HttpClient, private idle: Idle, private share: faShareService,private backend:HttpBackend) { 
  ...
  public getassetbsdata(keyvalue, page: number): Observable<any> {
    ...
    const headers = { 'Authorization': 'Token ' + token }
    let params: any = new HttpParams();
    return this.http.get<any>(faUrl + 'mstserv/searchbusinesssegment?query=' + keyvalue + '&page=' + page, { 'headers': headers });
  ...
  public getassetccdata(data: number): Observable<any> {
    ...
    return this.http.post<any>(faUrl + "mstserv/search_ccbs", d, { 'headers': headers });
...
public getDepreciationPrepare(data1,data2):Observable<any>{
 ...
 public getDepreciationForecastPrepare():Observable<any>{
  const getToken: any = localStorage.getItem('sessionData')
  let tokenValue = JSON.parse(getToken);
  let token = tokenValue.token
  const headers = { 'Authorization': 'Token ' + token }
  return this.http.get<any>(faUrl + "faserv/report_depreciation",{ 'headers': headers,responseType: 'blob' as 'json'  });
}

 // api depreciation_cal_Regularexcel_prepare
 public getDepreciationRegularPrepare():Observable<any>{
  const getToken: any = localStorage.getItem('sessionData')
  let tokenValue = JSON.parse(getToken);
  let token = tokenValue.token
  const headers = { 'Authorization': 'Token ' + token }
  return this.http.get<any>(faUrl + "faserv/report_depreciation_regular",{ 'headers': headers,responseType: 'blob' as 'json'  });
}
...
public getemployeedropdown( data:any,page:any) {
  ...
  const headers = { 'Authorization': 'Token ' + token };
  // console.log('dear=,', data)
  return this.http.get(faUrl + "usrserv/fetch_emp_dropdown?page="+page+'&data='+data , { 'headers': headers });
...
public getemployeeFKdd(empkeyvalue, pageno): Observable<any> {
  ...
  let token = tokenValue.token
  const headers = { 'Authorization': 'Token ' + token }
  return this.http.get<any>(faUrl + 'usrserv/searchemployee?query=' + empkeyvalue + '&page=' + pageno, { 'headers': headers })
}
}
