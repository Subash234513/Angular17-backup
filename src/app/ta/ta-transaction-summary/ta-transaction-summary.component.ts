...
export class TaTransactionSummaryComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (this.travelrole) {
      this.travelrole.forEach(element => {
        if (element.name == 'Maker') {
          ...
          this.tabs.push({ ind: 7, index: 6, name: 'Travel Report' });
          this.tabs.push({ ind: 6, index: 4, name: 'Requirement Report' });
    ...
    this.alltourform = this.fb.group({
      tourno: null,
      startdate: null,
      enddate: null,
      ...
      if (this.permitmatassetidauto && this.autocompletetrigger && this.permitmatassetidauto.panel) {
        fromEvent(this.permitmatassetidauto.panel.nativeElement, 'scroll').pipe(
          ...
          if (atBottom) {
            ...
            if (this.has_nextid) {
              this.taservice.getpermittedlist(this.empid, this.permitinputasset.nativeElement.value, this.has_presentid + 1, "").subscribe(data => {
                let dts = data['data'];
                console.log('h--=', data);
                console.log("SS", dts)
  ...
  dateadminsrch() {
    ...
    this.enterapproveaction(statusvalue)
  }
  touradminnosrch() {
    let statusvalue = this.touradminform.value.action
    this.enterapproveaction(statusvalue)
  ...
  enterapproveaction(index) {
    // this.rewuirementsdata
    // this.reqstatuslist
    ...
    let actionint = value.action
    var employee = value.employee;
    if (value.empcode) {
      employee = value.empcode;
    }
    console.log("BOOKS", bookingsts)
    ...
    this.index = index;
    // if (Number(bookingsts) <= -2 && actionint == 1){
    //   this.index = 2
  ...
  subModuleData(data, defind) {
    ...
    if (data) {
      if (data.index == 1) {
        ...
        this.gettraveltype()
        this.shareservice.report_requirement.next('1')
        ...
        // this.searchalltoursummary()
        this.gettraveldropdown()
        // this.alltoursummary.ngOnInit()
        ...
        this.recentchat = false;
        this.alltour = true;
        this.shareservice.all_tourrequirement.next('Travel')
  ...
  clearreportpage() {
    // this.tourreportform.value.tourno.clear()
    // this.tourreportform.value.requestdate.clear()
    this.tourreportform.get('tourno').reset();
    this.tourreportform.get('startdate').reset();
    this.tourreportform.get('enddate').reset();
    this.shareservice.report_startdate.next(null)
    this.shareservice.report_enddate.next(null)
  ...
  reportrequirementchange() {
    this.shareservice.report_requirement.next(this.tourreportform.value.requirement)
  ...
  gettraveltype() {
    this.taservice.gettourrequirementdropdown()
      .subscribe(res => {
        ...
        this.reqlist.splice(2, 2)
        // this.reqlist.splice(2,1)
  ...
  gettraveldropdown() {
    ...
    this.taservice.gettravelreportdropdown()
      .subscribe(res => {
        ...
        this.travelreportdropdown = this.travelreportdropdown.filter(function (element) {
          return element.name.toLowerCase() !== 'requirements'
        })
  ...
  reportexceldownload() {
    let reportname = this.tourreportform.value.requirement;
    console.log('tourno', this.tourreportform.value.tourno)
    let val


    // if(this.tourreportform.value.tourno=='' || this.tourreportform.value.tourno==null ){
    this.taservice.gettourreportxl(this.shareservice.report_tourpage.value, this.shareservice.report_requirement.value
      , this.shareservice.report_tournumb.value, this.shareservice.report_startdate.value,
      this.shareservice.report_enddate.value).subscribe(data => {
    ...
    // }
    // else{
    // this.taservice.getsearchtour_report_xl(this.tourreportform.value.tourno,this.shareservice.report_tourpage.value,this.tourreportform.value.requirement).subscribe(data => {
  ...
  alltoursearch() {
    // this.shareservice.all_tournumb.next(this.alltourform.value.tourno)
    // this.alltoursummary.ngOnInit()

  }
  searchalltoursummary() {
    // this.shareservice.all_tournumb.next(this.alltourform.value.tourno)
    if (this.alltourform.value.tourno == '') {
      this.shareservice.all_tournumb.next(null)
    }
    // else if (this.alltourform.value.reportdropdown != 'Travel'){
    //   return false;
    // }
    else {
      this.shareservice.all_tournumb.next(this.alltourform.value.tourno)
    }
    this.shareservice.all_tourstartdate.next(this.alltourform.value.startdate)
    this.shareservice.all_tourenddate.next(this.alltourform.value.enddate)
    this.shareservice.all_tourrequirement.next(this.alltourform.value.reportdropdown)
    this.alltoursummary.ngOnInit()
  }

  clearalltourpage() {
    this.alltourform.reset()
    this.alltourform.get('reportdropdown').setValue("Travel")

    this.shareservice.all_tournumb.next(null)
    this.shareservice.all_tourstartdate.next(null)
    this.shareservice.all_tourenddate.next(null)

    this.alltoursummary.ngOnInit()

  }
  datealltoursrch() {
    this.shareservice.all_tourstartdate.next(this.datePipe.transform(this.alltourform.value.startdate, 'dd-MMM-yyyy'))
    // this.alltoursummary.ngOnInit()
    console.log('transaction all tur date', this.datePipe.transform(this.alltourform.value.startdate, 'dd-MMM-yyyy'))

  }

  enddatealltourstch() {
    this.shareservice.all_tourenddate.next(this.datePipe.transform(this.alltourform.value.enddate, 'dd-MMM-yyyy'))

  }

  travelsummaryreportdownload() {
    // getsearchalltoursummarydownload

    this.taservice.getsearchalltoursummarydownload(this.alltourform.value.tourno, 1,
      this.datePipe.transform(this.alltourform.value.startdate, 'dd-MMM-yyyy'), this.datePipe.transform(this.alltourform.value.enddate, 'dd-MMM-yyyy'),
      this.alltourform.value.reportdropdown).subscribe(data => {
