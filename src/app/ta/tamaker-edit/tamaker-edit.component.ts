...
export class TamakerEditComponent implements OnInit, AfterViewInit {
  ...
  accBook(evt) {
    ...
    this.data_final = {
      ...
      "cost_per_head": Number(this.accomodationForm.value.cost_per_head),
      "issuance_type": this.accomodationForm.value.issuance_type,
      "difference_inamount": Number(this.accomodationForm.value.difference_inamount),
      "fare_quoted": Number(this.accomodationForm.value.fare_quoted),
  ...
  objectassign(tour) {
    let data = `&tour_no=${tour}&booking_status=-6`
    if (this.ngreload) {
      this.ngreload = false;
      this.taservice.getTourAdminSummary(3, 1, data, 0).subscribe(res => {
        let data = res["data"][0];
        let objects = {
          id: data['tourid'],
          status: data['status'],
  ...
  train(e) {
    ...
    this.taservice.gettrainlist(this.bookid)
      .subscribe((result) => {
        ...
        console.log("results", results['cancel_data'][0])
        this.cancellimit = new Date(results['booking_date']);
        let amt1 = results['ticket_amount']?Number(results['ticket_amount']):0
        let amt2 = results['ticket_amount_personal']?Number(results['ticket_amount_personal']):0
        this.bookedamount = amt1+amt2;
