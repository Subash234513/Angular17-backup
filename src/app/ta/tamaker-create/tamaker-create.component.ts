...
export class TamakerCreateComponent implements OnInit, AfterViewInit {
  ...
  submitForm() {
    ...
    if (this.locationreason == true) {
      if (this.taForm.value.non_base_location == '' || this.taForm.value.non_base_location == null) {
        this.notification.showError("Please enter explanation Non based location travel")
  ...
  EditsubmitForm() {
    ...
    if (this.locationreason == true) {
      if (this.taForm.value.non_base_location == '' || this.taForm.value.non_base_location == null) {
        this.notification.showError("Please enter explanation for Non based location travel")
        this.taForm.get('non_base_location').setValue(null)
    ...
    for (let i in creditdtlsdatas) {
      ...
      if (this.bookavail) {
        let length = creditdtlsdatas[i].requirements.length
        ...
        for (let j = 0; j < creditdtlsdatas[i].requirements.length; j++) {
          ...
          if (creditdtlsdatas[i].requirements[j].otherkey) {
            ...
            // if(creditdtlsdatas[i].requirements[j].booking_needed == '2'){
            //   if(creditdtlsdatas[i].requirements[j].travel_type_cab == ('' || null)){
            //     this.notification.showError('Please Choose Cab Type')
            //   throw new Error
            //   }
            // }
  ...
  deleterequirement(i, j, data) {
    console.log('deletereq', data.value)
    ...
    // console.log(this.taForm.value.detail[i].requirements[j].Accomodationkey)
    // this.taForm.get("detail")["controls"][i].get("requirements")["controls"][j].reset()
    ...
    // else {
    if (data.value.id) {
      this.particularrequirementdelete(data.value.id, data.value.booking_needed)
    }
  ...
  particularrequirementdelete(id, type) {
  ...
  minselectreq(i, j) {
    ...
    //   this.reqirementsdisable = false
    //   // this.taForm.value.detail[i].requirements.enable()
    ...
    //   }
    //   else {
    //     // console.log('endate >today date')
    //     this.reqirementsdisable = true
    //     // this.taForm.get('detail')['controls'][i].get('requirements').clear()
    //     this.tablewidthchange(i)
    //     return null
    //   }


    //   // this.taForm.value.detail[i].requirements.disable()
    //   return null
    // }
  ...
  bookingshow(data) {
    console.log('abcdefghijklmnopqrstuvwyxz.,')
    ...
    if (data.value.booking_needed == '1') {
      ...
      this.taservice.getacclist(data.value.id)
        .subscribe(res => {
          ...
          console.log('bookedformdae', this.bookedform.value)

        })
    }
    if (data.value.booking_needed == '2') {
      this.getfileuploaded(data.value.id, data.value.booking_needed)
    ...
    if (data.value.booking_needed == '3') {
      this.getfileuploaded(data.value.id, data.value.booking_needed)
    ...
    if (data.value.booking_needed == '4') {
      this.getfileuploaded(data.value.id, data.value.booking_needed)
    ...
    if (data.value.booking_needed == '5') {
      this.getfileuploaded(data.value.id, data.value.booking_needed)
  ...
  cityfromtosearch(ind, event) {
    ...
    if (this.tourreasonid != 9 && this.tourreasonid != 10) {
      ...
      // (this.taForm.get('detail') as FormArray).at(i).get('startingpoint').valueChanges
      //   .pipe(
      //     debounceTime(100),
      //     distinctUntilChanged(),
      //     tap(() => {
